/**
 ...
 @brief FatFS file system module include file  R0.10b (C) ChaN, 2014.
...
#if _USE_LFN
static
const BYTE LfnOfs[] =                             /* Offset of LFN characters in the directory entry */
{
...
/* Get LFN if available */
        if (dp->sect && fno->lfsize && dp->lfn_idx != 0xFFFF)
        {
            ...
            while ((w = *lfn++) != 0)             /* Get an LFN character */
            {
                ...
                if (!w)                           /* No LFN if it could not be converted */
                {
                    i = 0; break;
                }
                if (_DF1S && w >= 0x100)          /* Put 1st byte if it is a DBC (always false on SBCS cfg) */
                    p[i++] = (TCHAR)(w >> 8);
#endif
                if (i >= fno->lfsize - 1)         /* No LFN if buffer overflow */
                {
                    i = 0; break;
                }
                p[i++] = (TCHAR)w;
...
/* If dot entry is not exist, */
                    ...
                    {
/* it is the root directory and stay there */
...
/* and check if new object is exist */
